...
const BUS_ALERTS = "https://api.woodworthp.workers.dev/alerts/bus";
const RAIL_ALERTS = "https://api.woodworthp.workers.dev/alerts/rail";
...
function updateView() {
  ...
  if (systemwideAlerts.length > 0) {
    ...
    systemwideAlerts.forEach((alert, index) => {
      ...
      let systemwideAlertText = `<strong>${
        alert.headerText
      }:</strong> ${alert.descriptionText.replaceAll("\n", "<br>")}`;
